...
class ListInstancesResponseData(BaseModel):
    """
    ...
    gpu_vendor: Optional[StrictStr] = Field(default=None, description="Vendor of the GPU assigned to the instance, if any. Null when no GPU is attached.", alias="gpuVendor")
    gpu_model: Optional[StrictStr] = Field(default=None, description="Model of the GPU assigned to the instance, if any. Null when no GPU is attached.", alias="gpuModel")
    __properties: ClassVar[List[str]] = ["tenantId", "customerId", "additionalIps", "name", "displayName", "instanceId", "dataCenter", "region", "regionName", "productId", "imageId", "ipConfig", "macAddress", "ramMb", "cpuCores", "osType", "diskMb", "sshKeys", "createdDate", "cancelDate", "status", "vHostId", "vHostNumber", "vHostName", "addOns", "errorMessage", "productType", "productName", "defaultUser", "applicationId", "gpuVendor", "gpuModel"]
    ...
    def product_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['hdd', 'ssd', 'vds', 'nvme', 'performance', 'gpu']):
            raise ValueError("must be one of enum values ('hdd', 'ssd', 'vds', 'nvme', 'performance', 'gpu')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if gpu_vendor (nullable) is None
        # and model_fields_set contains the field
        if self.gpu_vendor is None and "gpu_vendor" in self.model_fields_set:
            _dict['gpuVendor'] = None

        # set to None if gpu_model (nullable) is None
        # and model_fields_set contains the field
        if self.gpu_model is None and "gpu_model" in self.model_fields_set:
            _dict['gpuModel'] = None
