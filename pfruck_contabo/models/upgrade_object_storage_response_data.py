...
class UpgradeObjectStorageResponseData(BaseModel):
    """
    ...
    object_storage_id: Annotated[str, Field(min_length=1, strict=True)] = Field(description="Object storage id", alias="objectStorageId")
    created_date: StrictStr = Field(description="Creation date for object storage.", alias="createdDate")
    data_center: StrictStr = Field(description="Data center of the object storage.", alias="dataCenter")
    auto_scaling: AutoScalingTypeResponse = Field(description="The autoscaling limit of the object storage.", alias="autoScaling")
    ...
    total_purchased_space_tb: Union[StrictFloat, StrictInt] = Field(description="Total purchased object storage space in TB.", alias="totalPurchasedSpaceTB")
    ...
    __properties: ClassVar[List[str]] = ["tenantId", "customerId", "objectStorageId", "createdDate", "dataCenter", "autoScaling", "s3Url", "status", "totalPurchasedSpaceTB", "region", "displayName"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['READY', 'PROVISIONING', 'UPGRADING', 'CANCELLED', 'ERROR', 'DISABLED', 'MANUAL_PROVISIONING', 'PRODUCT_NOT_AVAILABLE', 'LIMIT_EXCEEDED', 'VERIFICATION_REQUIRED', 'ORDER_PROCESSING', 'PENDING_PAYMENT', 'UNKNOWN']):
            raise ValueError("must be one of enum values ('READY', 'PROVISIONING', 'UPGRADING', 'CANCELLED', 'ERROR', 'DISABLED', 'MANUAL_PROVISIONING', 'PRODUCT_NOT_AVAILABLE', 'LIMIT_EXCEEDED', 'VERIFICATION_REQUIRED', 'ORDER_PROCESSING', 'PENDING_PAYMENT', 'UNKNOWN')")
        return value
